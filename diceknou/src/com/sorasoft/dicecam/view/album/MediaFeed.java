...
import com.sorasoft.dicecam.view.album.MediaClustering.Cluster;
            ...
            public void run() {
                ...
                while (ImageManager.isMediaScannerScanning(mContext.getContentResolver())) {
                    ...
                    try {
                        if (mContext == null)
                            return;
                        showToast("initializing" , Toast.LENGTH_LONG);
                        Thread.sleep(6000);
                ...
                if (mWaitingForMediaScanner) {
                    showToast("loading new", Toast.LENGTH_LONG);
                    mWaitingForMediaScanner = false;
                    if (dataSource != null) {
                        dataSource.loadMediaSets(feed);
                    }
...
//        if (mContext != null && !((Gallery) mContext).isPaused()) {
        	if (mContext != null) {
            ((ViewAfterPictureTaken) mContext).getHandler().post(new Runnable() {
                public void run() {
                    if (mContext != null) {
